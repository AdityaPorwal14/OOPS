...
private:
    // Data members
    unordered_map<string, int> items;  // Stores item names and their quantities
    string customerName;               // Name of the customer

public:
    // Constructor to set customer name
    ShoppingList(string name) : customerName(name) {}

    // Function to add or update an item in the shopping list
    void addOrUpdateItem(const string& itemName, int quantity) {
        if (quantity <= 0) {
            cout << "Invalid quantity for " << itemName << ". Please enter a positive value." << endl;
            return;
        }
        items[itemName] = quantity;
        cout << "Added/Updated " << itemName << " with quantity " << quantity << endl;
    }
    ...
    void removeItem(const string& itemName) {
        if (items.find(itemName) != items.end()) {
            ...
            cout << itemName << " is not in the shopping list." << endl;
        }
    }

    // Function to display the shopping list
    void displayList() const {
        cout << "Shopping List for " << customerName << ":" << endl;
        if (items.empty()) {
    ...
    // Function to get the total number of items in the shopping list
...
int main() {
    // Create a shopping list for a customer named "John"
    ShoppingList shoppingList("John");

    // Add or update items in the shopping list
    shoppingList.addOrUpdateItem("Apples", 5);
    shoppingList.addOrUpdateItem("Milk", 2);
    shoppingList.addOrUpdateItem("Bread", 1);

    // Display the current shopping list
    shoppingList.displayList();

    // Update quantity of an item
    shoppingList.addOrUpdateItem("Apples", 10);
